...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/11/13 14:05:44 by oriabyi           #+#    #+#             */
/*   Updated: 2018/11/26 14:02:19 by oriabyi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				set_nude_pieces_coord(t_filler *filler)
{
	...
	y = 0;
	filler->piece.start_x = INT32_MAX;
...
int					parse_line(t_filler *filler, char *line)
{
	if (ft_strstr(line, "Plateau "))
	{
		get_filler_info(line, filler, 1);
		fill_map(filler, 1);
	}
	else if (ft_strstr(line, "Piece "))
	{
		get_filler_info(line, filler, 3);
		fill_piece(filler);
...
int					main(void)
{
	...
	while (lgnl(0, &line) > 0)
	{
		...
		if (filler.my == '\0' && ft_strstr(line, "$$$")
			&& ft_strstr(line, "oriabyi.filler"))
		{
			filler.my = (line[10] == '1') ? (char)'O' : (char)'X';
			filler.enemy = (line[10] == '1') ? (char)'X' : (char)'O';
		}
		else if (parse_line(&filler, line) == 1)
			return (1);
		ft_strdel(&line);
	}
	return (0);
