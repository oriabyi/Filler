...
void				get_symbol(const char *line, char **ret, int code)
{
	int				c;
	...
	if (code == '2')
		*ret = ft_multjoinfr(3, NULL, *ret, BACK_DEFAULT);
}
...
static int			check_arguments(int ac, char **av, int *code)
{
	if (ac == 2)
	{
...
int					main(int ac, char **av)
{
	...
	if (check_arguments(ac, av, &code) == 1)
		return (1);
	vm = (t_vm_filler*)ft_memalloc(sizeof(t_vm_filler));
	vm->fd = 0;
	vm->ac = ac;
	vm->code = code;
	while (lgnl(0, &line) > 0)
